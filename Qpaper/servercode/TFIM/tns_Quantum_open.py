	...
	def add_batch(self):
		...
		for ind_setting in range(self.setting_number):
			for s in range(self.batch_size):
				istate = self.state2ind(self.mps.Give_Sample(self.direction_settings[ind_setting]))
...
for h in range(5):
	H = h / 5.
	space_size = 20
	sm = Laser_MPS(space_size, "random", "ising", J = 1., H = H, theta = pi/4)
	sm.cutoff = 0.00001
	sm.train(5)
	# sm.saveMPS()
