...
class training_set(object):
	def __init__(self, space_size, distri, **kwarg):
		...
		self.setting_number = self.space_size * 6 + 3
...
def successive_train(t, m, lstm, successive_fidelity, real_fidelity, notebook, i):
	t.add_batch()
	m.train(t, 1, 10)
	m.saveMPS(i)
	if i!=0:
		...
		notebook.append((t.set_size, t.setting_number, deepcopy(successive_fidelity), deepcopy(real_fidelity)))	
		stamp = './n%dl%d/' % (space_size, i)
		save(stamp+'notebook.npy',notebook)
...
for space_size in range(6, 30, 1):
	type = "cluster"
	sm = MPS(space_size, type)
	...
	for i in range(50):
		last_m = successive_train(t, m, last_m, successive_fidelity, real_fidelity)
		if i > 10 and successive_fidelity[-1] > 0.99999:
			break
