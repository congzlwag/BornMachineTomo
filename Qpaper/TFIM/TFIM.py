...
from numpy import zeros,ones,exp,sort,conj,array,argsort,log,dot,tensordot,outer,reshape,swapaxes,angle,pi,save,load
import numpy as np
...
class training_set(object):
	def __init__(self, space_size, distri, **kwarg):
		...
		self.setting_number = self.space_size * 2 + 1
...
class Laser_MPS(object):
	def __init__(self, space_size, initial_state_type, hamiltonian, **kwarg):
		self.energy = 1j
		self.space_size = space_size
		self.state_number = 2 ** space_size
		...
		if hamiltonian == "ising":
			...
			S_theta = np.cos(self.theta) * S_x + np.sin(self.theta) * S_y
		...
		if initial_state_type == "random":
			...
			for i in range(space_size):
				rs = rand(self.bond_dimension[(i - 1) % self.space_size], self.bond_dimension[i]) + rand(self.bond_dimension[(i - 1) % self.space_size], self.bond_dimension[i]) * 1.j
				r = array([rs, rs])
				r = swapaxes(r, 0, 1)
				self.matrices.append(r)

		elif initial_state_type == "sz":
			init_bond_dimension = 1
			self.bond_dimension = [init_bond_dimension] * self.space_size ## bond[i] connect i i+1
			self.matrices = []

			for i in range(self.space_size):
				self.matrices.append(np.array([1, 0]).reshape(1,2,1))

		elif initial_state_type == "sx":
			init_bond_dimension = 1
			self.bond_dimension = [init_bond_dimension] * self.space_size ## bond[i] connect i i+1
			self.matrices = []

			for i in range(self.space_size):
				self.matrices.append(np.array([1/np.sqrt(2), -1/np.sqrt(2)]).reshape(1,2,1))
	...
	def sweep(self):
		...
		for i in range(N - 2, -1, -1):
			...
			eigenValues, eigenVectors = eigsh(localhamiltonian, k = 1)
	...
	def train(self, loops):
		for i in range(loops):
			print("U  P",self.Give_Psi(array([1]*self.space_size), [0]*self.space_size))
			print("DOWN",self.Give_Psi(array([0]*self.space_size), [0]*self.space_size))
			self.sweep()
	...
	def saveMPS(self):
		stamp = 'n%dH%.1f' % (self.space_size, self.H)
		try:
			shutil.rmtree(stamp)
		except:
		...
		fp.write("space_size=%d\t,cutoff=%.10f\t\n"% (self.space_size, self.cutoff)) 
...
class MPS(object):
	...
	def saveMPS(self, succ_loop):
		stamp = 'N%dH%.1fL%d' % (self.space_size, self.H, succ_loop)
		try:
			mkdir('./'+stamp+'/')
		except:
			shutil.rmtree(stamp)
...
def successive_train(t, m, lstm, successive_fidelity, real_fidelity, i):
	...
	m.saveMPS(i)
	if i!=0:
		stamp = './N%dH%.1fL%d/' % (space_size, H, i)
		chdir('./'+stamp+'/')
		fp = open('MPS.log', 'a')
...
for h in range(1, 5):
	...
	H = h / 10.
	space_size = 16
	sm = Laser_MPS(space_size, "random", "ising", J = -1., H = H, theta = 0)
	sm.cutoff = 0.00001
	sm.train(20)
	print(sm.Give_Psi(array([1]*sm.space_size), [0]*sm.space_size))
	print(sm.Give_Psi(array([0]*sm.space_size), [0]*sm.space_size))
	
	# sm.saveMPS()
	
	# fp = open('Corre.log', 'w')
	# for site in range(4,12):
		# fp.write("Corre length of H=", H, "on site", site, " is ", sm.Give_Correlation_length(site,"z"))
	# fp.close()
	
	t = training_set(space_size, "MPSsampled", batch_size = 3, initial_batch_number = 1, mps = sm)
	m = MPS(space_size, "randomforTFIM", H = H)
	m.cutoff = 0.01
	...
	for i in range(0):
		...
		# if i > 20 and successive_fidelity[-1] > 0.99999:
			# break
