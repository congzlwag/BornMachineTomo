...
if __name__ == '__main__':
	...
	if typ=='random':
		space_size = 10
		measout_dir = measout_dir+'/%s/%d/%d/'%(typ,space_size,sd)
		wd = './%s/%d/%d/R%d/'%(typ,space_size,sd,rk)
	else:
		space_size = 30
		measout_dir = measout_dir+'/%s/%d/'%(typ,space_size)
	...
	ds.load("%s/R%dSet"%(measout_dir, rk))
	...
	if work_mode=='start':
		...
		if typ=='random':
			...
			iB = m.ibatch
			metaTrain(m, mxBatch-iB, nloop, True, Dmax=4)
		else:
			if space_size >= 20:
				for qq in range(5):
			...
			for qq in range(5):
				...
				metaTrain(m, 3, nloop, Dmax=3)
			iB = m.ibatch
			metaTrain(m, mxBatch-iB, nloop, True, cutoff=0.8, Dmax=2)
